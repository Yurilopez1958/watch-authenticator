...
type Card = { id: PlanId; price: string; priceYear: string; auth: string; val: string; dev: number; highlight?: boolean };
const CARDS: Card[] = [
  { id: 'free',     price: '0',     priceYear: '0',      auth: '5',   val: '10',  dev: 1 },
  { id: 'pro',      price: '9.99',  priceYear: '99.99',  auth: '100', val: '500', dev: 2, highlight: true },
  { id: 'business', price: '29.99', priceYear: '299.99', auth: '∞',   val: '∞',   dev: 5 },
];
  ...
  const subscribe = async (plan: 'pro' | 'business') => {
    ...
    setNotice(
      error && error !== 'server_error'
        ? error
        : t('No se pudo iniciar el pago ahora mismo. Inténtalo de nuevo en un momento.', 'Could not start checkout right now. Please try again in a moment.'),
    );
  };
          ...
          return (
            <div key={c.id} className={`card p-5 space-y-3 ${c.highlight ? 'border-accent' : ''}`}>
              <div className="flex items-baseline justify-between">
                ...
                <div className="text-right"><span className="text-2xl font-bold">{c.price === '0' ? t('Gratis', 'Free') : `$${interval === 'year' ? c.priceYear : c.price}`}</span>{c.price !== '0' && <span className="text-xs text-dim">/{interval === 'year' ? t('año', 'yr') : t('mes', 'mo')}</span>}</div>
              </div>
